...
func (d MailDir) Ensure() (err error) {
	...
	if err == nil {
		// create subdirs
		for _, subdir := range []string{DirNew, DirCur, DirTmp} {
...
func (d MailDir) Temp(fname string) (f string) {
	f = filepath.Join(d.Filepath(), DirTmp, fname)
	return
}
...
func (d MailDir) New(fname string) (f string) {
	f = filepath.Join(d.Filepath(), DirNew, fname)
	return
}

func (d MailDir) Cur(fname string) (f string) {
	f = filepath.Join(d.Filepath(), DirCur, fname)
	return
}
